...
export default function ConversationBody() {
	const { conversation } = usePatientConversations();
	...
	return (
		<div className="flex flex-col items-start w-full h-[80vh]">
			<div className="block overflow-y-auto overscroll-none p-4 w-full h-[60vh]">
				<div className="flex flex-col-reverse flex-grow gap-4 justify-end self-end py-12 w-full">
					<div>
						{conversation?.messages?.reverse().map((message) => {
							...
							return (
								<div
									...
									className={`py-1 flex gap-2 items-center justify-end ${message.fromAdmin && "flex-row-reverse"}`}
								>
